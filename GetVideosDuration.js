...
let pervStr = `${new Date(Date.now() - 86400000).toLocaleDateString('zh-CN')}--`;
...
let readFileList = async (dir, fmts) => {
    ...
    for (let index = 0; index < files.length; index++) {
        ...
        if (temp.filter(p => p.startsWith(".")).length > 0
            ...
            fs.appendFileSync(pervStr + scannedIngoreDirLog, fullPath + os.EOL, 'utf8')
        ...
        if (stat.isDirectory()) {
            ...
            // 如果是文件，写入日志，并判断类别
            fs.appendFileSync(pervStr + scannedFileLog, fullPath + os.EOL, 'utf8')
            ...
            if (isVideoArr.length > 0) {
                ...
                fs.appendFileSync(pervStr + scannedVideoLog, temp, 'utf8');
